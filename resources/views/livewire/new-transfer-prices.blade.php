                        ...
                        <x-card title="#{{$r->id}} - {{$r->name}} | FROM: {{$r->startingPoint->name}} | TO: {{$r->endingPoint->name}}">
                            <x-slot name="title">
                                <div class="flex gap-4 w-full justify-between ">
                                    <x-badge icon="refresh" lg  primary :label="$r->startingPoint->name.' -> '.$r->endingPoint->name" />
                            ...
                            <div x-show="isOpen">
                                ...
                                <div class="grid grid-cols-4  gap-2">
                                    ...
                                    <x-input wire:model.debounce.300ms="modelPrices.{{$r->id}}.discount" label="Discount in %" placeholder="0"/>

                                    <x-input wire:model.debounce.300ms="modelPrices.{{$r->id}}.commission" label="Commission in %" placeholder="0"/>
                                ...
                                <div class="grid grid-cols-4  gap-2 mb-4">
                                    <x-input  wire:model.debounce.500ms="modelPrices.{{$r->id}}.price"  label="One Way Price" />

                                    <x-input wire:model="modelPrices.{{$r->id}}.price_with_discount" label="Price with discount" disabled/>

                                    <x-input wire:model="modelPrices.{{$r->id}}.price_with_commission" label="Commission" disabled />
                                </div>
                                ...
                                @if(Arr::get($modelPrices,"{$r->id}.round_trip"))
                                    <div class="grid grid-cols-4  gap-2 mb-4">
                                        <x-input  wire:model.debounce.500ms="modelPrices.{{$r->id}}.price_round_trip" label="Round Trip Price" />

                                        <x-input wire:model="modelPrices.{{$r->id}}.round_trip_price_with_discount" label="Round Trip with discount" disabled/>

                                        <x-input wire:model="modelPrices.{{$r->id}}.round_trip_price_with_commission" label="Round Trip Commission" disabled />
                                    </div>
                            ...
                            <div class="flex gap-8" x-show="!open">
                                <span class="ds-badge ds-badge-ghost">Price: {{Arr::get($modelPrices,"{$r->id}.price")}}  €</span>
                                <span class="ds-badge ds-badge-ghost">Price w/ disc.: {{Arr::get($modelPrices,"{$r->id}.price_with_discount")}} €</span>
                                <span class="ds-badge ds-badge-ghost ">Price w/ comm.: {{Arr::get($modelPrices,"{$r->id}.price_with_commission")}} €</span>
                                @if(Arr::get($modelPrices,"{$r->id}.round_trip"))
                                @endif
                            </div>
